...
const HeaderResult_js_1 = require('./HeaderResult.cjs');
const ImageResult_js_1 = require('./ImageResult.cjs');
const InfoResult_js_1 = require('./InfoResult.cjs');
const ParseParameters_js_1 = require('./ParseParameters.cjs');
...
class PDFParse {
    ...
    async getHeader(check = false) {
        ...
        return await (0, HeaderResult_js_1.getHeader)(this.options.url, check);
    }
    /**
    ...
    async getPageText(page, parseParams, total) {
        const viewport = page.getViewport({ scale: 1 });
        const params = (0, ParseParameters_js_1.setDefaultParseParameters)(parseParams);
    ...
    async getImage(params = {}) {
        ...
        (0, ParseParameters_js_1.setDefaultParseParameters)(params);
        for (let i = 1; i <= result.total; i++) {
            if (this.shouldParse(i, result.total, params)) {
                const page = await doc.getPage(i);
